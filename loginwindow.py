...
class TitleLabel(QLabel):
    ...
    def paintEvent(self, event):
        ...
        scaled_flag = self.flag_image.scaled(flag_width, flag_height,
                                             Qt.AspectRatioMode.KeepAspectRatio,
                                             Qt.TransformationMode.SmoothTransformation)
        painter.drawPixmap(flag_x, flag_y, scaled_flag)

class LoginWindow(QDialog):  # QObject 제거
    ...
    def show_message(title, message, icon):
        ...
        msg_box.setFont(QFont("강한공군체", 15, QFont.Weight.Bold))
        # 스타일시트 적용
        ...
        if icon_label:
            ...
            if icon_pixmap:
                scaled_pixmap = icon_pixmap.scaled(22, 22,
                                                   Qt.AspectRatioMode.KeepAspectRatio,
                                                   Qt.TransformationMode.SmoothTransformation)
            ...
            if not table_exists:
                ...
                cursor.execute('''
                    CREATE TABLE IF NOT EXISTS users (
                        ...
                        UNIQUE(military_type, military_number)
                    )
                ''')
...
class RegisterDialog(QDialog):
    ...
    def initUI(self):
        ...
        for label_text, field in field_layouts:
            ...
            field_layout.addWidget(field, alignment=Qt.AlignmentFlag.AlignRight)
    ...
    def show_message(title, message, icon):
        ...
        msg_box.setText(message)
        msg_box.setIcon(icon)
        msg_box.setFont(QFont("강한공군체", 12, QFont.Weight.Bold))  # 글자 크기 5포인트 감소
        msg_box.setStyleSheet("""
            QMessageBox {
                background-color: #f0f0f0;
            }
    ...
    def register_user(self):
        ...
        try:
            ...
            if not all([military_type, military_number, name, username, password, confirm_password]):
                self.show_message(self.tr("입력 오류"), self.tr("모든 필드를 입력해주세요."), QMessageBox.Icon.Warning)
                return

            if password != confirm_password:
                self.show_message(self.tr("비밀번호 오류"), self.tr("비밀번호가 일치하지 않습니다."), QMessageBox.Icon.Warning)
                return
            ...
            if not self.validate_password(password):
                self.show_message(self.tr("비밀번호 오류"),
                                  self.tr("비밀번호는 다음 조건을 만족해야 합니다:\n"
                                          ...
                                          "- 영문 소문자 포함\n"
                                          "- 숫자 포함\n"
                                          "- 특수문자 포함"), QMessageBox.Icon.Warning)
                return
            ...
            if cursor.fetchone():
                self.show_message(self.tr("등록 오류"), self.tr("해당 군, 군번은 이미 등록되어 있습니다."), QMessageBox.Icon.Warning)
                return

            cursor.execute("SELECT username FROM users WHERE username = ?", (username,))
            if cursor.fetchone():
                self.show_message(self.tr("등록 오류"), self.tr("이미 존재하는 아이디입니다."), QMessageBox.Icon.Warning)
                return
            ...
            conn.commit()
            self.show_message(self.tr("등록 성공"), self.tr("사용자 등록이 완료되었습니다."), QMessageBox.Icon.Information)
...
class FindCredentialsDialog(QDialog):
    ...
    def find_id(self):
        ...
        try:
            ...
            if result:
                self.show_message(self.tr("아이디 찾기"), f"{found_id}: {result[0]}", QMessageBox.Icon.Information)
            else:
                self.show_message(self.tr("아이디 찾기"), self.tr("일치하는 사용자가 없습니다."), QMessageBox.Icon.Warning)
        finally:
            if conn:
                conn.close()
    ...
    def reset_password(self):
        ...
        try:
            ...
            if result:
                ...
                conn.commit()
                self.show_message(self.tr("비밀번호 초기화"), self.tr("비밀번호가 군번으로 초기화되었습니다."), QMessageBox.Icon.Information)
            else:
                self.show_message(self.tr("비밀번호 초기화"), self.tr("일치하는 사용자가 없습니다."), QMessageBox.Icon.Warning)

        except Exception as e:
            if conn:
                conn.rollback()
            self.show_message(self.tr("오류"), self.tr(f"오류가 발생했습니다: {str(e)}"), QMessageBox.Icon.Critical)
        finally:
            if conn:
                conn.close()
...
if __name__ == '__main__':
    ...
    window = LoginWindow()
    window.show()
    app.exec() # 불필요한 if문과 sys.exit() 제거
    if hasattr(window, 'username') and hasattr(window, 'db_name'):
